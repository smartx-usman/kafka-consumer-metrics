...
public class ParserTelegrafNet implements parsable {
    private static final Logger logger = LogManager.getLogger(ParserTelegrafNet.class);

    private final StoreRecordES store_record_es;
    private final StoreRecordPrometheus store_record_prometheus;
    private final File file = new File("/metrics/net_usage.json");
    private final ObjectMapper mapper = new ObjectMapper();
    private SequenceWriter sequence_writer = null;
    private FileWriter file_writer = null;

    public ParserTelegrafNet(StoreRecordES es, StoreRecordPrometheus prometheus) {
        store_record_es = es;
    ...
    public void parse_record(ConsumerRecord<String, String> record, String es_index) {
        try {
            ...
            String[] interface_label = measurement_plugin_labels[2].split("=");
            ...
            jsonMap.put(interface_label[0], interface_label[1]);
