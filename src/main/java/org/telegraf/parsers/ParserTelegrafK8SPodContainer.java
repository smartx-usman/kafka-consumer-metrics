...
public class ParserTelegrafK8SPodContainer implements parsable {
    private static final Logger logger = LogManager.getLogger(ParserTelegrafK8SPodContainer.class);
    private final StoreRecordES store_record_es;
    private final StoreRecordPrometheus store_record_prometheus;
    private final File file = new File("/metrics/kubernetes_pod_container.json");
    private final ObjectMapper mapper = new ObjectMapper();
    private FileWriter file_writer = null;
    private SequenceWriter sequence_writer = null;

    public ParserTelegrafK8SPodContainer(StoreRecordES es, StoreRecordPrometheus prometheus) {
        store_record_es = es;
    ...
    public void parse_record(ConsumerRecord<String, String> record, String es_index) {
        try {
            ...
            String[] container_name = measurement_plugin_labels[1].split("=");
            String[] host_label = measurement_plugin_labels[2].split("=");
            String[] namespace = measurement_plugin_labels[3].split("=");
            //String[] node_name = measurement_plugin_labels[4].split("=");
            String[] pod_name = measurement_plugin_labels[5].split("=");
            ...
            jsonMap.put(container_name[0], container_name[1]);
